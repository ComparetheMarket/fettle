...
namespace Fettle.Tests.Core.EndToEnd
{
    class Happy_path : Contexts.EndToEnd
    {
        ...
        {
            Assert.That(Result.SurvivingMutants.Count, Is.EqualTo(8),
                string.Join(Environment.NewLine, Result.SurvivingMutants.Select(sm =>
                    $"{Path.GetFileName(sm.SourceFilePath)}:{sm.SourceLine} \"{sm.OriginalLine}\" => \"{sm.MutatedLine}\"")));

            Assert.That(Result.SurvivingMutants.All(sm => sm.SourceFilePath.EndsWith(@"PartiallyTestedNumberComparison.cs")));

            Assert.That(Result.SurvivingMutants.SingleOrDefault(
                sm => sm.SourceLine == 7 &&
